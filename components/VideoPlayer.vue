  ...
  >
    ...
    <Transition name="fade">
      <div
        ...
        class="absolute top-0 left-0 right-0 z-[50] bg-gradient-to-b from-black/80 to-transparent px-4 py-3 pointer-events-none"
      >
        <div class="flex items-center justify-between">
          <!-- Back Button (Left) -->
          <button
            @click.stop="router.back()"
            class="pointer-events-auto flex items-center gap-2 text-white hover:text-gray-300 transition px-2 py-1"
          >
            <svg
              class="w-5 h-5"
              fill="none"
            ...
            <div class="relative volume-slider-container">
              ...
              <Transition name="fade">
                <div
                  ...
                  class="absolute top-full left-1/2 -translate-x-1/2 mt-2 bg-black/80 backdrop-blur-sm p-3"
                  @click.stop
                >
                  <input
                    type="range"
                    ...
                    class="w-2 h-24 bg-gray-700 rounded-lg appearance-none cursor-pointer volume-slider vertical-slider"
    ...
    <!-- Click to Show Overlays or Pause (TikTok-like) - Overlay -->
    <div
...
<script setup>
  ...
  const handleClickOutside = (e) => {
    if (!showQualityMenu.value && !showVolumeSlider.value) return
  ...
  const setupPlayer = () => {
    ...
    player.value.on("volumechange", () => {
      if (player.value) {
        ...
        // Mute only when volume = 0
        isMuted.value = player.value.volume === 0
        // Save to localStorage
        if (process.client) {
          localStorage.setItem("dramaqu_volume", player.value.volume.toString())
          localStorage.setItem("dramaqu_muted", (player.value.volume === 0).toString())
        }
      }
    })
  ...
  // Handle screen click - show overlays if hidden, pause if visible
  const handleScreenClick = (e) => {
    e.stopPropagation()
    e.preventDefault()

    // showOverlays: false = visible, true = hidden
    // If playing and overlays are hidden (showOverlays = true), show overlays (don't pause)
    if (!isPaused.value && props.showOverlays) {
      // Toggle to show overlays (set showOverlays to false = visible)
      emit("toggle-overlays")
      ...
      autoHideTimeout = setTimeout(() => {
        if (!isPaused.value && !props.showOverlays) {
          // Hide overlays again (set showOverlays to true = hidden)
          emit("toggle-overlays")
        }
      }, 5000)
